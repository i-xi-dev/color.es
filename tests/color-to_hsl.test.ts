...
Deno.test("Color.prototype.toHsl()", () => {
  for (const c of r1Cases) {
    const hsl = Color.fromRgb({ r: c.r, g: 0, b: 0 }).toHsl();
  ...
  for (const c of r2Cases) {
    const hsl = Color.fromRgb({ r: 255, g: c.x, b: c.x }).toHsl();
    assertStrictEquals(hsl.h.toFixed(2), c.h);
    assertStrictEquals(hsl.s.toFixed(2), c.s);
    assertStrictEquals(hsl.l.toFixed(4), c.l);
  }
  ...
  for (const c of r3Cases) {
    const hsl = Color.fromRgb({ r: c.r, g: c.g, b: c.b }).toHsl();
    assertStrictEquals(hsl.h.toFixed(2), c.h);
...
Deno.test("Color.prototype.toHsl({}) - discardAlpha", () => {
